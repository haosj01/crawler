...
class ConnectMySQL(object):
    ...
    def query_mysql(self, sql_sentence):
        ...
        try:
            num = cur.execute(sql_sentence)
            # 执行sql语句
            results = cur.fetchall()
            # 获取查询的结果
            logger.info("mysql查询成功：{}", sql_sentence)
            logger.info("mysql查询的数据量为：{}", num)
        except Exception as e:
            logger.error("mysql查询发生错误：{},sql语句：{}", e,sql_sentence)
            raise e
        finally:
            cur.close()
            # 关闭游标
    ...
    def insert_mysql(self, sql_sentence):
        ...
        try:
            ...
            conn.commit()
            # 提交
            logger.debug("mysql插入成功：{}", sql_sentence)
            logger.info("mysql插入的数据量为：{}", num)
        except Exception as e:
            logger.error("mysql插入发生错误：{},sql语句 ====>{}", e,sql_sentence)
            conn.rollback()
            # 回滚
        finally:
            cur.close()
